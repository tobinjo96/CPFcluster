...
def build_CCgraph(X, geo_neighbor_adjacency_matrix, min_samples, cutoff, n_jobs, distance_metric='euclidean'):
    ...
    Parameters:
        ...
        geo_neighbor_adjacency_matrix: The 0/1 adjacency matrix calculated from the geo_coordinates, 
            where 1 indicates that a point is among the k-nearest neighbors of another point.
    ...
    CCmat = CCmat.multiply(geo_neighbor_adjacency_matrix)
...
class CPFcluster:
    ...
    def fit(self, X, geo_neighbor_adjacency_matrix, k_values=None):
        """
        ...
        for k in k_values:
            ...
            components, CCmat, knn_radius = build_CCgraph(X, geo_neighbor_adjacency_matrix, k, self.cutoff, self.n_jobs, self.distance_metric)
            self.CCmat = CCmat
